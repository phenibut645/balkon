...
After the accepted job reward, admin resolving, and dashboard `/economy/me` read cleanup slices:
...
- Dashboard `/economy/me` should not call `ItemService.ensureMemberByDiscordId(...)` for balance read.
...
1. Sell inventory item to bot, later.
...
2. Member resolving cleanup in other non-economy services.
...
3. Dashboard route module extraction.
   - `dashboardRoutes.ts` remains a large legacy composition/handler file.
   - Extract only coherent route groups, not random one-off endpoints, and preserve response shapes.
...
Recommended order after the completed job reward, admin resolving, and dashboard economy read slices:

1. **ItemService inventory before market/bot-shop/sell-to-bot changes**
...
2. **Dashboard route module extraction inventory**
   - Goal: identify remaining endpoint groups in `dashboardRoutes.ts` and extract coherent modules by domain.
   - Why: API surface is growing and the legacy dashboard route file should become composition-only.
...
## 9. Completed Slice Note: Dashboard Economy Read Cleanup

Accepted implementation shape:

```text
dashboard /economy/me
  -> memberService.ensureMemberByDiscordId(auth user, { createdSource: "unknown" })
  -> EconomyService.getMemberBalancesByDiscordId(auth user)
  -> existing response shape
```

Preserved behavior:

- success response remains `ok: true` with `balance.odm` and `balance.ldm`;
- failure style remains `ECONOMY_LOAD_FAILED` with `Failed to load balance.`;
- no core economy/member/item service files were changed;
- no schema/migration change was added;
- no broader dashboard route extraction was performed.

Follow-up:

- `dashboardRoutes.ts` remains a broad legacy route file and should be decomposed by coherent route groups, not by random isolated endpoint churn.
