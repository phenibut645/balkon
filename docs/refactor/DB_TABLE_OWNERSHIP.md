...
- KAN-69 removed the direct `src/commands/menu.ts` balance-summary helper read; the `/menu` home balance chip and balance panel now read through the same tiny `EconomyService` method, but the broader `menu.ts` screen orchestration is still a high-risk delegated surface.
