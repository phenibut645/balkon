...
| --- | --- | --- | --- | --- | --- | --- |
| `src/commands/balance.ts` | `/balance` | Reads `members.balance` and `members.ldm_balance` through `dataBaseHandler.getFromTable`. | Command contains direct DB helper usage; read-only economy surface. | economy/bot command | should move to economy/profile read service later | medium |
...
| `src/commands/menu.ts` | `/menu` | Opens main interaction menu and reads balances via `dataBaseHandler.getFromTable` near line 3299. | Large command/UI file with direct DB helper read. | bot UI/economy view | likely more behavior inside large file than this inventory details | medium |
...
| `src/core/EconomyService.ts` | Economy totals/snapshots/market capitalization and admin balance adjustment. | Intended economy owner but currently not all balance mutations flow through it. | economy | some member resolving still depends on item/member helpers | high |
...
| `src/core/LocaleService.ts` | Locale setting lookup/update using `DataBaseHandler` and one `pool.query`. | User preference persistence; low direct blast radius. | locale/user settings | none | medium |
...
| `src/core/EconomyService.ts` | 3 calls: 106, 126, 145 | Economy totals and snapshots. | Intended economy owner, mostly read/snapshot here. | economy | admin adjustment uses transaction/connection not in pool count | medium |
...
| `src/core/LocaleService.ts` | 1 call: 53 | Locale preference update/read path. | Direct setting write, limited scope. | locale | none | medium |
...
| `src/api/routes/dashboard/adminStreamerRoutes.ts` | 2, 68 | Imports `DataBaseHandler` for failure handling around streamer archive response. | Route module still depends on legacy response helper. | admin/streamers | low-risk cleanup candidate | medium |
| `src/commands/balance.ts` | 2, 24, 25 | Reads balances from `members` through `dataBaseHandler.getFromTable`. | SQL-like data helper usage in command. | economy/bot command | should become service read | medium |
| `src/commands/menu.ts` | 20, 3299, 3300 | Reads balances for main menu through `dataBaseHandler.getFromTable`. | Large command UI file has direct DB helper dependency. | bot UI/economy view | large file contents not fully mapped here | medium |
...
| `src/core/LocaleService.ts` | 10 refs | Uses `DataBaseHandler` response/error helpers. | Small user settings flow depends on legacy helper. | locale/settings | cleanup candidate | medium |
...
| Replace route-level `DataBaseHandler.isFail` response check in admin streamer route | `src/api/routes/dashboard/adminStreamerRoutes.ts` | Local cleanup of legacy response helper usage around one service call. | Small behavior-preserving removal candidate after usage inventory. | admin/streamers | must preserve exact response shape | medium |
