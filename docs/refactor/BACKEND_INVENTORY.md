...
| `src/events/interactionCreate.ts` | `interactionCreateController(interaction)` | Syncs Discord profile through `memberService.ensureMemberFromDiscordProfile`, syncs interaction guild membership through `guildMemberService.ensureInteractionGuildMember(...)`, checks private-command permissions through `permissionController(...)` with an explicit `permissionResponse.success === false || !permissionResponse.data` gate, and dispatches slash commands/autocomplete/selects/buttons/modals to command handlers. | Central bot adapter for member lifecycle, guild membership, permission checks, and command execution. | bot interactions/member/guild permissions | `PermissionController` still returns `DBResponse<boolean>` and still uses `DataBaseHandler.errorHandling(...)` internally; KAN-74 changed only the adapter-level helper dependency in this file | high |
...
| `src/events/interactionCreate.ts` | no current `DataBaseHandler` usage | KAN-74 removed the adapter-level helper dependency; the file no longer imports `DataBaseHandler`, no longer calls `DataBaseHandler.isFail(...)`, and now uses `permissionResponse.success === false || !permissionResponse.data` after `permissionController(...)`. | bot/member/guild permissions | `PermissionController` internals still use `DataBaseHandler.errorHandling(...)`; member/profile sync and guild-member sync remain delegated to `MemberService` and `GuildMemberService` | low |
...
| `src/events/interactionCreate.ts` | Main Discord interaction adapter. | Member profile sync, guild-member sync, permission checks, command dispatch, component routing. | bot interactions/member/guild permissions | ownership is still split across services, and permission internals remain legacy-shaped even though the adapter-level `DataBaseHandler` helper dependency is resolved | high |
