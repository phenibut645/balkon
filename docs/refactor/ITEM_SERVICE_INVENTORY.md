...
- Kept market, bot shop buy/sell mutations, inventory mutations, craft execution, craft recipe writes, OBS/service-item, and other write flows in `ItemService` for later slices.
...
| Public market | `listPublicMarket` | 1906-1989 | Read all public listings with seller/original owner display metadata and item metadata | `item_public_market`, `member_items`, `members`, `items`, `item_types`, `item_rarities` | Error helper only | Join read | None | Reads display cache from `members` | None | None | Uses private display-name mapper | API `/market`, menu market | `DBResponse<PublicMarketListingView[]>` | `MarketReadModel` | Low-medium | Market read model | Maybe later |
| Public market | `listUserPublicMarket` | 1991-2001 | Filter public market listings by seller Discord ID | Indirect same as `listPublicMarket` | In callee only | In callee only | None | None | None | None | Inefficient full-list filter | Menu/user market views | `DBResponse<PublicMarketListingView[]>` | `MarketReadModel` | Medium | Market read model | Not first |
...
- Market searches can follow in domain-specific read models.
